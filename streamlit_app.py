...
if "shuffled_problems_lcm" not in st.session_state:
    st.session_state.shuffled_problems_lcm = random.sample(problems, len(problems))
...
# --------------------------
# Problem Display
if st.session_state.lcm_index < len(st.session_state.shuffled_problems_lcm):
    a, b = st.session_state.shuffled_problems_lcm[st.session_state.lcm_index]
    st.subheader(f"📘 Problem {st.session_state.lcm_index + 1} of {len(st.session_state.shuffled_problems_lcm)}")
    ...
    if st.button("🔁 Start Over"):
        del st.session_state.shuffled_problems_lcm
        del st.session_state.lcm_index
        del st.session_state.lcm_score
        st.rerun()
